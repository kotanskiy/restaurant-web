...
<html>
    ...
    <body>
        ...
        <div class="navbar navbar-inverse">
                ...
                <h1 class="style-color">Список ингредиентов в блюде:</h1>
                <table class="style-color" border="1px">
                    <tr>
                        <th>Название ингредиента</th>
                        <th>Кол-во</th>
                    </tr>    
                    <c:forEach items="${ingredientsWithCount}" var="ingredient">
                        <tr>
                        <td>${ingredient.getName()} </td>   
                        <td>${ingredient.getCount()} <a href="${idDish}/ingredientindishdelete/${ingredient.getId()}">Удалить все ингредиенты</a></td>    
                             
                        </tr>
                    </c:forEach>
                </table>
                <h3>Добавить в это блюдо:</h3>
                <c:forEach items="${allIngredients}" var="ingredient">
                    Название - ${ingredient.getName()} <a href="${idDish}/addingredientindish/${ingredient.getId()}">Добавить этот ингредиент в блюдо</a> <br>
                </c:forEach>
            </div>
        </center> 
    </body>
